...
- **LLM chat** ([src/llm/](src/llm/)): a framework-agnostic domain like `music`. `OllamaClient` (singleton `ollamaClient`) is the Ollama HTTP transport and owns a `SerialQueue` (concurrency 1) so **every inference runs one-at-a-time process-wide** — a hard constraint for the CPU-only host. `Assistant` (singleton `assistant`) is the use-case: it sends the system prompt ([src/llm/system-prompt.txt](src/llm/system-prompt.txt), imported as text via Bun's `with { type: 'text' }`) + one user message (no history) with an Ollama structured-output JSON schema and returns a validated `AssistantResult` (`action: play | stop | chat | reject`). The schema/system-prompt are the guardrails. [src/events/messageCreate.ts](src/events/messageCreate.ts) is the Discord adapter: it answers only `@mention`s on non-reply guild messages, executes music actions via `musicService`, and replies with `allowedMentions: { parse: [] }`. The shared music use-case `MusicService` (singleton `musicService`, [src/music/MusicService.ts](src/music/MusicService.ts)) wraps resolve→(stream-or-download)→enqueue, returns a discriminated `PlayResult` (`{ ok: false, reason: 'live-unsupported' }` for live content), and is used by both `/play` and the chat adapter. Inference runs at low temperature with a deliberately small `num_ctx` (default 4096, also enforced server-side via the `ollama` service's `OLLAMA_CONTEXT_LENGTH`) — a large context makes Ollama allocate a KV cache big enough to OOM-kill the model runner on a CPU host. The bot's persona/behavior is edited in the prompt file alone, no code change needed.

- **Music playback** ([src/music/](src/music/)): after `YtDlpService` resolves a query, `MusicService` branches on the track — **live streams are rejected** (returns `{ ok: false }`), tracks over `streamThresholdSec` (env `MUSIC_STREAM_THRESHOLD_SEC`, default 600s) are **streamed** straight from a `yt-dlp | ffmpeg` pipe with no file on disk, and shorter ones are **downloaded** to `music_cache/` and reused on replay. A `QueuedTrack` carries a discriminated `PlaybackSource` (`{ kind: 'file' }` or a lazy `{ kind: 'stream', open() }`); [GuildPlayer](src/music/GuildPlayer.ts) opens the source **at play time** (not when queued) and feeds `@discordjs/voice` as `StreamType.OggOpus`. **Hard constraint:** no Opus encoder dependency (`@discordjs/opus`/`opusscript`) is installed, so playback must hand Discord **pre-encoded Ogg/Opus** — the file path uses yt-dlp's `--audio-format opus`, and the streaming path transcodes to Opus with ffmpeg's `libopus` (`-c:a libopus -f opus`) for the same reason. Never rely on discord.js to transcode arbitrary audio (it would reach for the missing encoder and crash). `streamTrack` reaps its spawned yt-dlp/ffmpeg processes when the returned stream is torn down (e.g. on `/stop`).
